describe("verifyEbInternalEventSourceName tests", () => {
  ...
  it("EB_INTERNAL_EVENT_SOURCE_NAME variable not set, throws an error", async () => {
    // Arrange
    const { verifyEbInternalEventSourceName } = await import(
      "./verify-eb-internal-event-source-name"
    );
  ...
  it("EB_INTERNAL_EVENT_SOURCE_NAME variable is set, does not throw an error", async () => {
    // Arrange
