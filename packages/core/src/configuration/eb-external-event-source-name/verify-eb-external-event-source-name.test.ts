describe("verifyEbExternalEventSourceName tests", () => {
  ...
  test("EB_EXTERNAL_EVENT_SOURCE_NAME variable not set, throws an error", async () => {
    // Arrange
    const { verifyEbExternalEventSourceName } = await import(
      "./verify-eb-external-event-source-name.js"
    );
  ...
  test("EB_EXTERNAL_EVENT_SOURCE_NAME variable is set, does not throw an error", async () => {
    // Arrange
